...
def load_and_inspect(dataset_name: str, config: Optional[str] = None, split: str = "train"):
    # Pass arguments explicitly so Pylance can match the correct overload signature
    ds = load_dataset(path=dataset_name, name=config, split=split)
...
def stream_dataset(dataset_name: str, config: Optional[str] = None, max_rows: int = 5):
...
if __name__ == "__main__":
    ...
    # FIXED: Updated identifier to include the proper namespace
    ds = load_and_inspect("cornell-movie-review-data/rotten_tomatoes", split="train")

    print("\n--- 2. Stream a dataset ---")
    # FIXED: Updated identifier here as well
